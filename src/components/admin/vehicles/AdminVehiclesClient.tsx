      ...
      <div className="md:hidden space-y-3">
        {filteredVehicles.length === 0 ? (
          <div className="text-center py-12 bg-card border border-border rounded-3xl p-6">
            <Car className="w-8 h-8 mx-auto stroke-1 text-muted-foreground/50 mb-2" />
            <p className="text-sm font-semibold">No vehicles found</p>
            <p className="text-xs text-muted-foreground mt-0.5 mb-4">No cars match your search criteria.</p>
            <Button
              variant="outline"
              size="sm"
              onClick={() => {
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="w-16 h-12 bg-muted/60 rounded-xl overflow-hidden shrink-0 border border-border/50 shadow-inner flex items-center justify-center">
                    {primaryImg ? (
                      <img
                        src={primaryImg}
                        alt={`${v.brand} ${v.model}`}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge className={STATUS_BADGE_MAP[v.status] || 'bg-muted text-muted-foreground'}>
                        {v.status}
                      </Badge>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-2 py-2 px-3 bg-muted/30 rounded-2xl text-xs">
                  <div>
                    <span className="text-[10px] text-muted-foreground block">Registration</span>
                    <span className="font-mono font-bold text-foreground text-[11px]">{v.registration_number}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] text-muted-foreground block">Daily Rate</span>
                    <span className="font-black text-foreground">₹{v.daily_rate?.toLocaleString('en-IN')}<span className="text-[10px] font-normal text-muted-foreground">/day</span></span>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-1 border-t border-border/60 gap-2">
                  <div className="text-[11px] text-muted-foreground truncate">
                    {v.branch ? `${v.branch.city} • ${v.branch.name}` : 'Main Branch'}
                  </div>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem asChild>
                          <Link href={`/cars/${v.id}`} target="_blank">
                            <Eye className="w-3.5 h-3.5 mr-2" /> View Public Page
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleUpdateStatus(v.id, 'available')}>
                          <CheckCircle2 className="w-3.5 h-3.5 mr-2 text-emerald-500" /> Set Available
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleUpdateStatus(v.id, 'maintenance')}>
                          <Wrench className="w-3.5 h-3.5 mr-2 text-orange-500" /> Set Maintenance
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => openDeleteModal(v)} className="text-rose-600">
                          <Trash2 className="w-3.5 h-3.5 mr-2" /> Delete Vehicle
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </div>
